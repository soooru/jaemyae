...
- **이스터에그**: **보라색이 되는 클릭(6번째, 누적 클릭 수가 6의 배수일 때)마다** 잼얘 순위표
  (rank) 모달이 바로 열린다 — 그 클릭은 코인 삽입으로 처리되지 않고 모달만 연다. 다음 클릭은
  자동으로 빨강부터 다시 시작(별도 리셋 변수 없이 `coinClickCount % 6`으로 자연스럽게 순환).
  시간 간격 조건은 없음 — 천천히 눌러도 6번째면 항상 열린다. (이전엔 "1.1초 이내 7연속 클릭"이라는
  타이밍 조건이 있었으나, 실제로는 트리거되는 걸 체감하기 어렵고 색 순환과 별개로 동작해 헷갈린다는
  피드백으로 2026-09-16에 이렇게 단순화함 — `src/main.ts`의 `coinClickCount` 참고, 별도 스트릭
  변수는 삭제됨.)
...
- **채용공고(`job`)** — 메인 하단 알약 버튼 `인력급구!인력급구!인력급구!` 클릭으로 진입점. `급 구`
  ...
  이야기 등록이라는 농담 설정 — `done` 모달의 "작업 완료!" 문구가 이 흐름을 전제로 한다).
...
- **잼얘 채우기(`form`)** — 필드 3개 전부 필수(빈 값 있으면 등록 버튼 비활성): `작업자`(20자),
  ...
  추가하고 `done` 모달로 전환.
- **작업 완료(`done`)** — 마스코트 + "작업 완료! / 잼얘 하나가 들어갔어요. 이제 자판기에서 뽑힐 수
  있어요." + 버튼 1개(`하나 더`, `form`으로 재진입).
